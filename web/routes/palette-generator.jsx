...
export default function () {
  ...
  const [hexCopied, setHexCopied] = useState('');
  ...
  // if (image) {
  //   // console.log("HAHAHAHHAHAAHAHAAHH");
  //   // encodeImage(image)
  //   // fetchColors();
  // }

  const handleCopy = (hexCopied) => {
    navigator.clipboard.writeText(hexCopied).then(() =>
      {
        setHexCopied('hexCode copied')
      }).catch(() => {
        setHexCopied("failed to copy")
      })
  ...
  const fetchColors = async () => {
    ...
    const result = await api.getColoursOpenAI({ image: imageB64, numColours: numColours, });
    ...
    <div id="paletteContainer">
      <div id="topText">
        <div id="replaceimage">
          ...
          <input type="file" id="photo" name="photo" accept="image/*" style={{ display: 'none' }} onChange={handleFileChange}></input>

        </div>
        <div className="colour-count">
          <label className = "num-color-label">Number of Colours:</label>
          <input type="number" min="1" max="10" value={numColours} onChange={(event) => setNumColours(event.target.valueAsNumber)} id="myRange"></input>
        </div>
        <button className="browse-btn" onClick={fetchColors} style={{ color: '#555151', 
                         padding: '0px 32px',
                         marginTop: '0px',
                         background: 'none',
                         color: 'rgba(255, 255, 255, 1)',
      ...
      <div id="colours">
        {hexCodes.map((hexCode, index) => {
          const brightness = calculateBrightness(hexCode);
          const textColor = brightness > 128 ? '#353535' : '#EFEFEE';
          
        return (
            <div className="colour" key={index} style={{ backgroundColor: hexCode }}>
              <p className="hexcodenames" onClick={() => handleCopy(hexCode)}>{hexCode}</p>
            </div>
          );
        })}
      </div>
